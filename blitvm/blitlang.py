...
class DangerousValue(Exception):
    """When you try to pass in something dangerous to blit, it won't catch everything though"""

    def __init__(self, *args):
        super().__init__(*args)


class BlitRuntimeError(Exception):
    """Something caused the Blit code to crash"""
...
def safe_range(*args):
    """limit the range of range()"""
    if len(args) > 3:
        raise TypeError("range expected at most 3 arguments, got {}".format(len(args)))
    if len(args) == 3:
        if (args[1] - args[0]) // args[2] > MAX_STRING_LENGTH:
            raise MemoryError("Sorry, I will not evaluate a range that long.")
    if len(args) == 2:
        if args[1] - args[0] > MAX_STRING_LENGTH:
            raise MemoryError("Sorry, I will not evaluate a range that long.")
    if len(args) == 1:
        if args[0] > MAX_STRING_LENGTH:
            raise MemoryError("Sorry, I will not evaluate a range that long.")
    return range(*args)


safe_range.__name__ = "range"
safe_range.__qualname__ = "range"
safe_range.__module__ = "builtins"
...
# track all the sandboxes created so can calculate memory usage
sandboxes = set()
...
class BlitEval(object):
    ...
    def eval(self, expr):
        ...
        try:
            ...
            exc = e
            node = None
            if hasattr(exc, "_blit_node"):  # pragma: no branch
                node = exc._blit_node
            raise BlitRuntimeError(exc, node=node) from exc
    ...
    def _eval(self, node):
        ...
        except (Return, Break, Continue, BlitRuntimeError):
            raise
        except Exception as e:
            exc = e
            if not hasattr(exc, "_blit_node"):  # pragma: no branch
                exc._blit_node = node
            raise BlitRuntimeError(exc, node=node) from exc
    ...
    def _eval_lambda(self, node):
        ...
        def _func(*args, **kwargs):
            ...
            func_spec = inspect.getfullargspec(_func)

            # Initialize the local_scope dictionary
            local_scope = {}

            # Populate varargs if present
            if func_spec.varargs is not None:
                local_scope[func_spec.varargs] = args

            # Populate keyword-only arguments and positional arguments
            for kwo in func_spec.kwonlyargs + func_spec.args:
                if kwo in kwargs:
                    local_scope[kwo] = kwargs.pop(kwo)

            # Populate varkw (keyword arguments) if present
            if func_spec.varkw is not None:
                local_scope[func_spec.varkw] = kwargs
    ...
    def _eval_excepthandler(self, node):
        ...
        if isinstance(exc, UNCATCHABLE_EXCEPTIONS) or (
            isinstance(exc, BlitRuntimeError)
    ...
    def track(self, node):
        ...
        self.size = len(repr(self.scope.dicts[-1]))
        # Don't do this. It's too slow.
        # self.size = len( json.dumps( self.scope.dicts[-1], default=repr, indent=0, separators=(",", ":")) + json.dumps( self._last_eval_result, default=repr, indent=0, separators=(",", ":")))
        sizesum = sum(s.size for s in sandboxes)
        if len(repr(self._last_eval_result)) > MAX_SCOPE_SIZE:
            raise MemoryError(
                f"Scope has used too much memory: {len(repr(self._last_eval_result))} > {MAX_SCOPE_SIZE}"
            )

        if sizesum > MAX_SCOPE_SIZE:
            for s in sandboxes:
                print("=" * 100)
                print(s.expr)
                print("dict", s.scope.dicts[-1])
                print("last result", s._last_eval_result)
            raise MemoryError(
                f"Scope has used too much memory: {sizesum} > {MAX_SCOPE_SIZE}"
            )
        if self.size > self.max_scope_size:
            raise Exception(
                f"Sandbox scope has exceeded max size: {self.max_scope_size}"
            )
...
def blit_eval(expr, scope=None, call_stack=None, module_dict=None):
    """Simply evaluate an expresssion"""
    module_dict = module_dict or {}
    modules = make_modules(module_dict)

    s = BlitEval(scope=scope, modules=modules, call_stack=call_stack)
    return s.eval(expr)


class BlitCoverage(BlitEval):
    ...
    def eval(self, expr):
        ...
        return super(BlitCoverage, self).eval(expr)

    def track(self, node):
        super(BlitCoverage, self).track(node)
        if hasattr(node, "seen_nodes"):
            print(f"Tracking seen_nodes {node} {node.__class__.__name__}")
            print(f"self.seen_nodes {self.seen_nodes}")
            print(f"node.seen_nodes {node.seen_nodes}")
...
def blit_test_coverage(expr, scope=None, call_stack=None, module_dict=None):
    """Run all test_* function in this expression"""

    modules = make_modules(module_dict or {})

    s = BlitCoverage(scope=scope, modules=modules, call_stack=call_stack)
    s.eval(expr)
    test_names = [n for n in s.scope if n.startswith("test_") and callable(s.scope[n])]
    for name in test_names:
        res = s.scope[name]()
        print(f"Testing {name}: {res}")
