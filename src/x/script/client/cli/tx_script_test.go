...
func (s *IntegrationTestSuite) TestCreateScript() {
	var (
		net    = s.network(nil)
...
func (s *IntegrationTestSuite) TestUpdateScript() {
	var (
		net    = s.network(nil)
		val    = net.Validators[0]
		ctx    = val.ClientCtx
	...
	for _, tc := range tests {
		s.T().Run(tc.desc, func(t *testing.T) {
			...
			args = append(args, fields...)
			args = append(args, tc.args...)
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdUpdateScript(), args)
			if tc.err != nil {
				require.ErrorIs(t, err, tc.err)
				return
			}
